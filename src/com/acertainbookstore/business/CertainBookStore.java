...
public class CertainBookStore implements BookStore, StockManager {
	...
	/*
	 * Validates the rating (0-5) and checks whether the ISBN is valid or not
	 */
	private synchronized void validate(BookRating rating) throws BookStoreException {
		...
		validateISBNInStock(rating.getISBN()); // Check if the book has valid ISBN and in stock
	}
	...
	public synchronized List<Book> getTopRatedBooks(int numBooks) throws BookStoreException {
		...
		if (numBooks > this.bookMap.size()) {
			throw new BookStoreException(BookStoreConstants.BOOK_NUM_PARAM + " " + numBooks + BookStoreConstants.INVALID + ": more than avaliable books");
		}
		
		List<BookStoreBook> sortedBooks = Arrays.asList(this.bookMap.values().toArray(new BookStoreBook[0]));
		
		Collections.sort(sortedBooks, new Comparator<BookStoreBook>() {
	        @Override
	        public int compare(BookStoreBook bsb1, BookStoreBook bsb2) {
	        	return Float.compare( bsb2.getAverageRating(), bsb1.getAverageRating() );
	        }
	    });
				
		List<BookStoreBook> firstKRated = sortedBooks.subList(0, numBooks);
		for(BookStoreBook book : firstKRated) {
			if(book.getAverageRating() == -1) {
				throw new BookStoreException("Not enough book ratings");
			}
		}
			
		return firstKRated.stream().map(book -> book.immutableBook()).collect(Collectors.toList());
	}
	...
	public synchronized void rateBooks(Set<BookRating> bookRating) throws BookStoreException {
		...
		for (BookRating br : bookRating) {
			validate(br); // Check whether the book is in stock.
			validateISBNInStock(br.getISBN()); // Check if the book has valid ISBN and in stock
		}
